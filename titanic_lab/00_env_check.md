...
**1) 주제별 “완성형 프롬프트 예시” (바로 사용 가능)**

**(A) 주제: Series vs DataFrame, index/columns**

`너는 파이썬 데이터분석 입문 튜터야.
아래 [주제]를 복습할 수 있는 실습형 문제를 난이도별로 3개(하/중/상) 만들어줘.

조건:
- 정답(코드/결과) 제공 금지
- 각 문제마다 힌트 2~3개(접근 방식 중심)
- Pandas 사용, 출력은 DataFrame/Series 확인
- 문제는 짧고 명확하게

[주제]
Series와 DataFrame 차이, index/columns 개념
...
조건:
1) 정답(코드/결과) 절대 제공하지 말 것
2) 각 문제는 dtype 확인 → 변환까지 포함
3) 힌트는 “무엇을 먼저 확인하고, 어떤 함수로 변환할지” 중심으로 2~3개
4) errors 옵션(coerce 등)을 활용해야만 풀 수 있는 문제를 최소 1개 포함

[주제]
dtype 확인 및 형 변환: 문자열→숫자, 문자열→날짜
...
**(C) 주제: 결측치(Null/NaN) 탐지/처리(isna, fillna, dropna) + 왜곡 방지**
...
조건:
- 정답(코드/결과) 제공 금지
- 결측치 탐지(isna/sum) + 처리(fillna/dropna) 모두 포함
- 각 문제마다 “처리 방식 선택 이유”를 묻는 문장을 1개 포함
- 힌트는 접근 방식 위주 2~3개

[주제]
결측치 탐지 및 처리 전략(삭제 vs 대체), 처리 후 분석값 비교
...
**(D) 주제: 파생 피처(derived feature) 생성 + groupby 집계**
...
조건:
- 정답(코드/결과) 제공 금지
- 파생 피처(예: sales=price*qty) 만들기 포함
- groupby 집계가 들어간 문제를 최소 2개 포함
- 힌트는 “어떤 컬럼을 만들고 → 어떤 기준으로 묶을지” 중심으로 2~3개

[주제]
파생 피처 생성 + groupby 집계(메뉴별/날짜별)
...
**(E) 주제: 파일 입출력(CSV/Excel/JSON) 저장 옵션(index, encoding, orient)**
...
- 정답(코드/결과) 제공 금지
- 저장/불러오기 모두 포함
- 최소 1문항은 “옵션을 왜 그렇게 설정해야 하는지” 설명을 요구
- 힌트는 ‘어떤 함수 + 어떤 옵션’을 떠올리게 하는 방식으로 2~3개

[주제]
CSV/Excel/JSON 입출력과 옵션(index, encoding, orient)
...
![png](00_env_check_files/00_env_check_26_1.png)
